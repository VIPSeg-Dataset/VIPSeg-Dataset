...
#        exit()
        ...
        if self.training:
            ...
            out = self.inference(batched_inputs, images, pred_centers, pred_regions, pred_weights, encode_feat)
            #torch.cuda.synthronize()
            #end = time.time()
...
#        print(pred_center.size())
    ...
    @torch.no_grad()
    def inference(self, batch_inputs, images, pred_centers_, pred_regions_, pred_weights_, encode_feats):
        """
    ...
    #        print(len(results))
            ...
            encode_feat = encode_feats[img_idx].unsqueeze(0)
 
            feat_shape = encode_feat.shape
            encode_feat = encode_feat.reshape(*feat_shape[:2], -1)
            result_instance = None

            pred_regions = [_pred[img_idx].unsqueeze(0) for _pred in pred_regions_]
            pred_weights = [_pred[img_idx].unsqueeze(0) for _pred in pred_weights_]
            pred_centers = [_pred[img_idx].unsqueeze(0) for _pred in pred_centers_]
            pool_size = [3,3,3,5,5]
...
#            processed_results.append({"sem_seg": pred_sem_seg, "instances": result_instance})
            ...
            if self.panoptic_combine:
                _,_,result_panoptic = self.combine_thing_and_stuff(
...
#        exit()
    ...
    def combine_thing_and_stuff(
            ...
            if keep.sum() > 0:
                ...
                for _idx, (_mask, _cate, _score) in enumerate(zip(pred_thing, thing_cate, thing_score)):
                    ...
                    if (cat_id_,True) not in dic_tmp:
                        ...
                        dic_tmp[(cat_id_,True)].append(current_segment_id)
                    else:
        ...
        for tmp_, curr_seg_id_list in dic_tmp.items():
            ...
            if isthing:
                for ii,cur_seg_id in enumerate(curr_seg_id_list):
                    new_id = cat_id_*100+ii
        ...
        return panoptic_seg, segments_info, panoptic_seg_mask
