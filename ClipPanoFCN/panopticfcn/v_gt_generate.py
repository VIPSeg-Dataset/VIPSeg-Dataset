...
class VideoGenerateGT(object):
    ...
    def assign_multi_scale(self, gt_instances, gt_semantics, image_shape, inst_outsize, feat_shape,frame_num):
        """
        ...
        gt_sem = gt_sem.permute(0, 3, 1, 2).float()
        #print(gt_sem.size())
        ...
        #print(new_w)
        #exit()
        gt_sem = F.interpolate(gt_sem, size=(new_h, new_w), 
                               mode='bilinear', align_corners=False)

        #print(gt_sem.size())
        #print(torch.unique(gt_sem))
        #exit()
        gt_semantics = torch.zeros(frame_num,self.stuff_classes, *inst_outsize, device=self.device)
        #print(gt_semantics.size())
...
#            print(gt_instances.gt_masks.tensor.size())
            bit_masks, mass_centers, scales = VideoGenerateGT.decode_bitwise_mask(common_stride=self.common_stride,
                                                                             gt_masks=gt_instances.gt_masks,
                                                                             device=self.device,
                                                                             use_mass=use_mass)
#            print(bit_masks.size())
#            print(mass_centers.size())
#            print(mass_centers)
#            print(scales)
#            print('*'*100)
#            exit()
...
#            print(gt_boxes_raw.tensor.size())
#            print(gt_class_raw.size())
#            print(gt_areas_raw.size())
#            exit()
            if use_mass:
            ...
            for layer_idx in range(len(feat_shape)):
                gt_center_per, gt_inst_per, gt_index_per, index_mask_per, classes_per = \
                            self._label_assignment(gt_class_raw, bit_masks, gt_centers_raw, 
                                                   ...
                                                   feat_shape[layer_idx],frame_num)

                gt_semscore_per, gt_semseg_per, sem_mask_per, index_mask_sem_per  = \
                            self._sem_assign(gt_semantics, feat_shape[layer_idx],frame_num)
            ...
            for layer_idx in range(len(feat_shape)):
                gt_center_per = torch.zeros(self.thing_classes,frame_num, *feat_shape[layer_idx], device=self.device)
                gt_inst_per = self.ignore_val * torch.ones(self.tensor_dim,frame_num, *inst_outsize, device=self.device)
                gt_index_per = torch.zeros(self.tensor_dim, frame_num,device=self.device)
                index_mask_per = torch.zeros(self.tensor_dim,frame_num, device=self.device)
    ...
    def _label_assignment(self, gt_class_raw, gt_masks_raw, gt_centers_raw, gt_boxes_raw, gt_areas_raw, inst_scale, image_shape, inst_outsize, feat_shape,frame_num):
        """
...
#        print(gt_areas_raw.size())
#        gt_areas_raw = gt_areas_raw.sum(1)/gt_areas_raw.size(1)
        
        gt_scoremap = torch.zeros(self.thing_classes, frame_num,*feat_shape, device=self.device)
        gt_instance = self.ignore_val * torch.ones(self.tensor_dim, frame_num,*inst_outsize, device=self.device)
        gt_index = torch.zeros(self.tensor_dim, frame_num,device=self.device)
        inst_mask = torch.zeros(self.tensor_dim, frame_num,device=self.device)
        ...
        gt_assign_mask_ = gt_assign_mask
        gt_assign_mask = gt_assign_mask.sum(1).bool()

#        print(gt_assign_mask)
#        print(gt_assign_mask.size())
        ##print(inst_scale[0])
        ##print(inst_scale[1])
        #print('*'*100)
        #exit()
        ...
        #print(gt_centers_raw)
        #print(gt_centers_raw.size())
        # assign center
        ...
        tmptmp = centers_int[..., 1] * feat_shape[1] + centers_int[..., 0]
        #print(centers_int.size())

        inst_mask[:num_inst] = gt_assign_mask_[gt_assign_mask]
        # assign classes
        ...
        #print(wh)
        VideoGenerateGT.generate_score_map(gt_scoremap, classes, wh,
    ...
    def _sem_assign(self, gt_semantic, feat_shape,frame_num): 
        """
        Assign labels for Stuff to each FPN stage.
        """
        gt_semantic_ = gt_semantic.permute(1,0,2,3)
        inst_mask = torch.zeros(self.stuff_classes,frame_num, device=self.device)

        gt_sem_labels = torch.zeros_like(gt_semantic_)
        gt_scoremap = F.interpolate(gt_semantic_, size=feat_shape, mode='bilinear', align_corners=False)
        gt_scoremap = gt_scoremap.clamp(max=1.0)
        gt_scoremap[gt_scoremap<0.5] = 0.0
        gt_assign_mask = gt_scoremap.reshape(self.stuff_classes,frame_num, -1).sum(dim=-1) > 0

        gt_assign_mask_ = gt_assign_mask
        gt_assign_mask = gt_assign_mask.sum(1).bool()

        num_sem = gt_assign_mask.sum()
        gt_sem_labels[:num_sem] = gt_semantic_[gt_assign_mask]
        gt_sem_masks = torch.zeros_like(gt_scoremap)
        gt_sem_masks[:num_sem] = gt_scoremap[gt_assign_mask].bool().float()
        inst_mask[:num_sem,:] = gt_assign_mask_[gt_assign_mask]
        return gt_scoremap, gt_sem_labels, gt_sem_masks, inst_mask

    @torch.no_grad()
    def generate(self, batched_input, images, features, inst_feat, frame_num,test_only=False):
        """
        ...
        gt_semantics = [x["v_sem_seg"] for x in batched_input]
        ...
        gt_center_labels, gt_inst_labels, gt_index_labels, index_mask_labels, gt_class_labels, \
                ...
                multi_apply(self.assign_multi_scale, gt_instances, gt_semantics,
                            ...
                            feat_shape=feat_shape,frame_num = frame_num)
    ...
    def decode_bitwise_mask(common_stride, gt_masks, device, use_mass=False):
        ...
        bit_masks = F.interpolate(bit_masks.float(), size=(new_h, new_w),
                                  mode="bilinear", align_corners=False)
        # center type [x, y]
        if use_mass:
            for _masks in bit_masks:
                m_tmp = []
                for _mask in _masks:
                    if _mask.sum() ==0:
                        mass_center = torch.Tensor([0.,0.])
                        mass_center = mass_center.to(bit_masks.device)
 
                        m_tmp.append(mass_center[[1,0]])
                    else:
                        mass_center = torch.nonzero(_mask.float(), as_tuple=False)
                        mass_center = mass_center.float().mean(dim=0)
                        m_tmp.append(mass_center[[1,0]])
                m_tmp = torch.stack(m_tmp)
                mass_centers.append(m_tmp)
    ...
    def generate_score_map(fmap, gt_class, gt_wh, centers_int, min_overlap, sigma_factor=6, device=None):
        ...
        radius = VideoGenerateGT.get_gaussian_radius(gt_wh, min_overlap)
        ...
        for i in range(gt_class.shape[0]):
            ...
            VideoGenerateGT.draw_gaussian(fmap[channel_index], centers_int[i], radius[i],
    ...
    @staticmethod
    def draw_gaussian(fmaps, centers, radiuss, k=1, sigma_factor=6, device=None):
        """
        Draw gaussian-based score map.
        """
        for fmap,center,radius in zip(fmaps,centers,radiuss):
            if center.sum()==0:
                continue    
            else:
                diameter = 2 * radius + 1
                gaussian = VideoGenerateGT.gaussian2D((radius, radius), sigma=diameter / sigma_factor)
