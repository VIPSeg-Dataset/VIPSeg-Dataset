...
def load_video_cityscapes_vps_json(json_file, image_dir, gt_dir, semseg_dir):
    """
    Args:
    ...
    for ann in json_info["annotations"]:
        ...
        # function to support other COCO-like datasets.
            image_file = os.path.join(image_dir, videoid_img_dic[video_id][image_id]['file_name'])
            image_files.append(image_file)
            
            label_file = os.path.join(gt_dir, image["file_name"])
...
def register_video_cityscapes_vps_json(
    ...
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_video_cityscapes_vps_json(
...
_PREDEFINED_SPLITS_CITYSCAPES_VPS = {
    "cityscapes_vps_video_train": (
        "cityscapes_vps/train/img",
        "cityscapes_vps/train/panoptic_video",
        "cityscapes_vps/video_panoptic_gt_train_city_vps.json",
        "cityscapes_vps/train/labelmap",
    ),
    "cityscapes_vps_video_val": (
        "cityscapes_vps/val/img",
        "cityscapes_vps/val/panoptic_video",
        "cityscapes_vps/video_panoptic_gt_val_city_vps.json",
        "cityscapes_vps/val/labelmap",
    ),
}


def register_all_video_cityscapes_vps(root):
    for (
        prefix,
        ...
        register_video_cityscapes_vps_json(
            prefix,
            metadata,
...
register_all_video_cityscapes_vps(_root)
