...
from panopticfcn.target_generator import VideoTwoframePanopticDeepLabTargetGenerator
...
class PanopticDatasetVideoTwoframeMapper:
    """
...
#            augs.append(ColorAugSSDTransform(img_format=cfg.INPUT.FORMAT))
        ...
        panoptic_target_generator = VideoTwoframePanopticDeepLabTargetGenerator(
            ignore_label=meta.ignore_label,
...
#        assert self.train_clipnum < video_length
        choice_index = np.random.choice(video_length-1,1)        
        ...
        for idx in [choice_index,choice_index+1]:
...
#       ut_semantic_seg=[]
    ...
    #            input_semantic_seg.append(sem_seg_gt.unsqueeze(0).long())
        tmp_ = np.stack(input_panoptic_seg)
    #        if not(len(np.unique(tmp_))==1 and np.unique(tmp_)[0]==0):
    #            break
                
        dataset_dict["image0"] = input_images[0]
        dataset_dict["image1"] = input_images[1]
